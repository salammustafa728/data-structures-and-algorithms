...
// Solution code here...
  let idx = null;
  if (arr.length > 0) {
    let strIndx = arr.reduce((a, b) => {
      return a.length > b.length ? a : b;
    });
    idx = arr.indexOf(strIndx);
  }
  else {
    idx = -1;
  }
...
Write a function named firstLetters that takes in an array of strings and returns an array containing only the first
...
const firstLetters = (arr) => {
  // Solution code here...
  let firstIndx = arr.map(letterval => letterval[0]);
  return firstIndx;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let val1 = arr.map(num => {
    let val2 = num.replace(/\D/g, '');
    return val2;
  });
  return val1;
};
