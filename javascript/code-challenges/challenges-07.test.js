...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  const newArr = starWarsArr.sort((a,b)=>Number(b.height) - Number(a.height));
  return newArr;
};
...
Your function should use slice within a loop and return an array of each successive string result from losing letters
 to the sharpener, until nothing is left.
...
const howMuchPencil = (str) => {
  ...
  for(let i=0;i<str.length;i++){
    newStr=newStr.slice(1,newStr.length).join('');
    result.push(newStr);
    newStr=newStr.split('');
  }
  // Solution code here...
...
Write a function name wordsToCharList that, given a string as input, returns a new array where every element
 is a character of the input string.
...
Write a function named listFoods that takes in the recipe and returns a new array of the food items without any amount
or units. Just the name. For example, '1 cup flour' will return 'flour'.
