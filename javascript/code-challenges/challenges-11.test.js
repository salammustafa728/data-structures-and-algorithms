...
function transformToLis(obj){
  // Solution code here...
  return Object.entries(obj).map(val=>`<li>${val.join(': ')}</li>`);
...
Write a function named count that, given an integer and an array of arrays, uses either filter, map, or reduce to count
the amount of times the integer is present in the array of arrays.
...
Write a function that, given an array of integer arrays as input, calculates the total sum of all the elements in the
array.

You may want to use filter, map, or reduce for this problem, but are not required to. You may need to use the same
...
const totalSum = (input) => {
  ...
  let sumTotal=  input.reduce((sum,value)=>{return sum+=value.reduce((sum,val)=>sum+=val);},0);
  return sumTotal;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(newArr=> newArr.filter(val=>(typeof val === 'number' && val % 5 ===0)).map(val2=> Math.pow(2,val2)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(val => val.gender === 'male' || val.gender=== 'female').map(val2 => val2.name).join(' and ');
};

/* ------------------------------------------------------------------------------------------------
CHALLENGE 6

Write a function named findShortest that, given the Star Wars data from Challenge 6, uses any combination of filter, map 
and reduce to return the name of the character who is the shortest in height.
------------------------------------------------------------------------------------------------ */

let findShortest = (data) => {
  // Solution code here...
  return data.reduce((minChar, val) => Number(minChar.height) < Number(val.height) ? minChar : val).name;
};
